...
def _extract_squashfs(file_path, tmp_dir):
    return execute_shell_command(f'cd {tmp_dir} && {FIND_SQUASHFS_TOOL_PATH} {file_path}')


def _extract_kernel_image(file_path, tmp_dir):
    return execute_shell_command(f'{UNPACK_KERNEL_TOOL_PATH} {file_path} {tmp_dir}/kernel_decompressed.img')
