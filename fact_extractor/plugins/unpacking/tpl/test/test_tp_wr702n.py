...
class TestTpWr702n(TestUnpackerBase):
    ...
    def test_get_meta_dict(self):
        ...
        assert expected_meta_data == self.firmware_container.get_meta_dict()

    def test_header_and_binary(self):
        self.unpacker.extract_files_from_file(self.test_firmware, self.tmp_dir.name)
        expected_files_with_hash = {
            'container_header.hdr': '446d194c0e5beeccf9cd1534205cbeedcedf28491beaeb72d85e485f61071e0a',
        ...
        for file, expected_hash in expected_files_with_hash.items():
            self._hash_compare(file, expected_hash)

    def _hash_compare(self, filename, hashsum):
        binary_data = get_binary_from_file(os.path.join(self.tmp_dir.name, filename))
        assert get_sha256(binary_data) == hashsum, f'Checksum incorrect for file {filename}'

    def test_extraction(self):
        in_file = self.test_firmware
        files, _ = self.unpacker.extract_files_from_file(in_file, self.tmp_dir.name)
