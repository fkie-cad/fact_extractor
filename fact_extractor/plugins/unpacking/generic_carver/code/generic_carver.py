...
def unpack_function(file_path, tmp_dir):
    ...
    output = execute_shell_command(f'binwalk --extract --carve --signature --directory  {shell_escape_string(str(tmp_dir))} {shell_escape_string(str(file_path))}')
...
class ArchivesFilter:
    ...
    def _remove_invalid_archives(self, file_path: Path, command, search_key=None):
        output = execute_shell_command(command.format(shell_escape_string(str(file_path))))
...
def _find_trailing_data_index_zip(file_path: Path) -> int | None:
    ...
    output = execute_shell_command(f'7z l {shell_escape_string(str(file_path))}')
...
def _find_trailing_data_index_bz2(file_path: Path) -> int | None:
    output = execute_shell_command(f'bzip2 -t {shell_escape_string(str(file_path))}')
